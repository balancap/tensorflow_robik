...
__global__ void __launch_bounds__(1024, 2)
    HexRotDepthwiseConv2DGPUKernelNHWC(const HexRotDepthwiseArgs args, const T* input,
...
EIGEN_DEVICE_FUNC bool CanLaunchHexRotDepthwiseConv2dGPUSmall(
...
EIGEN_DEVICE_FUNC bool CanLaunchHexRotDepthwiseConv2dBackpropFilterGPUSmall(
    const HexRotDepthwiseArgs args, const int block_rows) {
  return false && args.depth_multiplier == 1 && args.stride == 1 && args.in_rows <= 32 &&
         args.in_cols <= 32 && args.in_rows == args.out_rows &&
         args.in_cols == args.out_cols && args.pad_rows >= 0 &&
         args.pad_rows < args.filter_rows && args.pad_cols >= 0 &&
...
// use this kernel if CanLaunchHexRotDepthwiseConv2dGPUSmall(args) returns true.
...
// rotated by 180°.
template <typename T, HexRotDepthwiseConv2DDirection kDirection,
          int kKnownFilterWidth, int kKnownFilterHeight, int kBlockSlices,
          bool kKnownEvenRows>
__global__ __launch_bounds__(1024, 2) void HexRotDepthwiseConv2DGPUKernelNHWCSmall(
    const HexRotDepthwiseArgs args, const T* input, const T* filter, T* output) {
  assert(CanLaunchHexRotDepthwiseConv2dGPUSmall(args));
...
template <typename T, int kKnownFilterWidth, int kKnownFilterHeight,
          int kKnownDepthMultiplier>
__global__ void __launch_bounds__(1024, 2)
    HexRotDepthwiseConv2DGPUKernelNCHW(const HexRotDepthwiseArgs args, const T* input,
                                 const T* filter, T* output, int num_outputs) {
  const int in_rows = args.in_rows;
  const int in_cols = args.in_cols;
  const int in_depth = args.in_depth;
...
template <typename T, HexRotDepthwiseConv2DDirection kDirection,
          int kKnownFilterWidth, int kKnownFilterHeight, int kBlockSlices,
          bool kKnownEvenRows>
__global__ __launch_bounds__(1024, 2) void HexRotDepthwiseConv2DGPUKernelNCHWSmall(
    const HexRotDepthwiseArgs args, const T* input, const T* filter, T* output) {
  assert(CanLaunchHexRotDepthwiseConv2dGPUSmall(args));
...
template <typename T, HexRotDepthwiseConv2DDirection kDirection,
          int kKnownFilterWidth, int kKnownFilterHeight, int kBlockSlices,
          bool kKnownEvenRows>
void LaunchHexRotDepthwiseConv2dGPUSmall(const GpuDevice& d, const HexRotDepthwiseArgs args,
                                   const T* input, const T* filter, T* output,
                                   TensorFormat data_format) {
  const int block_rows = (args.in_rows + 1) / 2;
  dim3 block_dim;
  void (*kernel)(const HexRotDepthwiseArgs, const T*, const T*, T*);
  if (data_format == FORMAT_NHWC) {
    block_dim = dim3(kBlockSlices, args.in_cols, block_rows);
    kernel = HexRotDepthwiseConv2DGPUKernelNHWCSmall<T, kDirection, kKnownFilterWidth,
                                               kKnownFilterHeight, kBlockSlices,
                                               kKnownEvenRows>;
  } else if (data_format == FORMAT_NCHW) {
    block_dim = dim3(args.in_cols, block_rows, kBlockSlices);
    kernel = HexRotDepthwiseConv2DGPUKernelNCHWSmall<T, kDirection, kKnownFilterWidth,
                                               kKnownFilterHeight, kBlockSlices,
                                               kKnownEvenRows>;
  } else {
...
void LaunchHexRotDepthwiseConv2dGPUSmall(const GpuDevice& d, const HexRotDepthwiseArgs args,
                                   ...
                                   TensorFormat data_format) {
  if (args.in_rows & 1) {
    LaunchHexRotDepthwiseConv2dGPUSmall<T, kDirection, kKnownFilterWidth,
...
void LaunchHexRotDepthwiseConv2dGPU(const GpuDevice& d, const HexRotDepthwiseArgs args,
                              const T* input, const T* filter, T* output,
                              TensorFormat data_format) {
  void (*kernel)(const HexRotDepthwiseArgs, const T*, const T*, T*, int);
  if (data_format == FORMAT_NHWC) {
    kernel =
    ...
    kernel =
        HexRotDepthwiseConv2DGPUKernelNCHW<T, kKnownFilterWidth, kKnownFilterHeight,
                                     kKnownDepthMultiplier>;
  } else {
    assert(false && "Incorrect data format");
    return;
...
void LaunchHexRotDepthwiseConv2dGPU(const GpuDevice& d, const HexRotDepthwiseArgs args,
  ...
  if (args.depth_multiplier == 1) {
    if (CanLaunchHexRotDepthwiseConv2dGPUSmall(args)) {
      LaunchHexRotDepthwiseConv2dGPUSmall<T, DIRECTION_FORWARD, kKnownFilterWidth,
...
void LaunchHexRotDepthwiseConvOp<GPUDevice, T>::operator()(OpKernelContext* ctx,
  ...
  if (args.filter_rows == 3 && args.filter_cols == 3) {
    LaunchHexRotDepthwiseConv2dGPU<T, 3, 3>(d, args, input, filter, output,
                                      data_format);
  } else {
    LaunchHexRotDepthwiseConv2dGPU<T, -1, -1>(d, args, input, filter, output,
                                        data_format);
  }
...
template <typename T, int kKnownFilterWidth, int kKnownFilterHeight,
          int kKnownDepthMultiplier>
__global__ void __launch_bounds__(640, 2)
    HexRotDepthwiseConv2DBackpropInputGPUKernelNCHW(const HexRotDepthwiseArgs args,
                                              const T* out_backprop,
                                              const T* filter, T* in_backprop,
                                              int num_in_backprop) {
  const int in_rows = args.in_rows;
...
void LaunchHexRotDepthwiseConv2dBackpropInputGPU(const GpuDevice& d,
  ...
  if (data_format == FORMAT_NHWC) {
    ...
    kernel = HexRotDepthwiseConv2DBackpropInputGPUKernelNCHW<
        T, kKnownFilterWidth, kKnownFilterHeight, kKnownDepthMultiplier>;
  } else {
    assert(false && "Incorrect data format");
    return;
...
void LaunchHexRotDepthwiseConv2dBackpropInputGPU(const GpuDevice& d,
  ...
  if (args.depth_multiplier == 1) {
    if (CanLaunchHexRotDepthwiseConv2dGPUSmall(args)) {
      LaunchHexRotDepthwiseConv2dGPUSmall<T, DIRECTION_BACKWARD, kKnownFilterWidth,
                                    kKnownFilterHeight>(
...
void LaunchHexRotDepthwiseConvBackpropInputOp<GPUDevice, T>::operator()(
    OpKernelContext* ctx, const HexRotDepthwiseArgs& args, const T* out_backprop,
...
// CanLaunchHexRotDepthwiseConv2dGPUSmall(args) returns true.
...
__global__
__launch_bounds__(1024, 2) void HexRotDepthwiseConv2DBackpropFilterGPUKernelNHWCSmall(
    const HexRotDepthwiseArgs args, const T* output, const T* input, T* filter) {
  assert(CanLaunchHexRotDepthwiseConv2dBackpropFilterGPUSmall(args, blockDim.z));
  // Holds block plus halo and filter data for blockDim.x depths.
  extern __shared__ __align__(sizeof(T)) unsigned char shared_memory[];
  T* const shared_data = reinterpret_cast<T*>(shared_memory);
...
template <typename T, int kKnownFilterWidth, int kKnownFilterHeight,
          int kKnownDepthMultiplier>
__global__ void __launch_bounds__(640, 2)
    HexRotDepthwiseConv2DBackpropFilterGPUKernelNCHW(const HexRotDepthwiseArgs args,
                                               const T* out_backprop,
                                               const T* input,
                                               T* filter_backprop,
                                               int num_out_backprop) {
...
template <typename T, int kKnownFilterWidth, int kKnownFilterHeight,
          int kBlockSlices, int kAccumPixels>
__global__
__launch_bounds__(1024, 2) void HexRotDepthwiseConv2DBackpropFilterGPUKernelNCHWSmall(
    const HexRotDepthwiseArgs args, const T* output, const T* input, T* filter) {
  assert(CanLaunchHexRotDepthwiseConv2dBackpropFilterGPUSmall(args, blockDim.x));
  // Holds block plus halo and filter data for blockDim.z depths.
  extern __shared__ __align__(sizeof(T)) unsigned char shared_memory[];
  T* const shared_data = reinterpret_cast<T*>(shared_memory);
...
template <typename T, int kKnownFilterWidth, int kKnownFilterHeight,
          int kBlockSlices, int kAccumPixels>
bool TryLaunchHexRotDepthwiseConv2dBackpropFilterGPUSmall(
    const GpuDevice& d, const HexRotDepthwiseArgs args, const int block_rows,
  ...
  if (data_format == FORMAT_NHWC) {
    block_dim = dim3(kBlockSlices, args.in_cols, block_rows);
    kernel = HexRotDepthwiseConv2DBackpropFilterGPUKernelNHWCSmall<
        T, kKnownFilterWidth, kKnownFilterHeight, kBlockSlices, kAccumPixels>;
  } else if (data_format == FORMAT_NCHW) {
    block_dim = dim3(args.in_cols, block_rows, kBlockSlices);
    kernel = HexRotDepthwiseConv2DBackpropFilterGPUKernelNCHWSmall<
        T, kKnownFilterWidth, kKnownFilterHeight, kBlockSlices, kAccumPixels>;
  } else {
    assert(false && "Incorrect data format");
    return false;
...
bool TryLaunchHexRotDepthwiseConv2dBackpropFilterGPUSmall(
  ...
  if (block_pixels > 512) {
    return TryLaunchHexRotDepthwiseConv2dBackpropFilterGPUSmall<
...
template <typename T, int kKnownFilterWidth, int kKnownFilterHeight>
bool TryLaunchHexRotDepthwiseConv2dBackpropFilterGPUSmall(
    const GpuDevice& d, const HexRotDepthwiseArgs args, const T* out_backprop,
  ...
  if (!CanLaunchHexRotDepthwiseConv2dBackpropFilterGPUSmall(args, block_rows)) {
    return false;
  }
...
void LaunchHexRotDepthwiseConv2dBackpropFilterGPU(const GpuDevice& d,
  ...
  if (data_format == FORMAT_NHWC) {
    ...
    kernel = HexRotDepthwiseConv2DBackpropFilterGPUKernelNCHW<
        T, kKnownFilterWidth, kKnownFilterHeight, kKnownDepthMultiplier>;
  } else {
    assert(false && "Incorrect data format");
    return;
...
void LaunchHexRotDepthwiseConv2dBackpropFilterGPU(const GpuDevice& d,
                                            ...
                                            TensorFormat data_format) {
  if (args.depth_multiplier == 1) {
    if (TryLaunchHexRotDepthwiseConv2dBackpropFilterGPUSmall<T, kKnownFilterWidth,
                                                       kKnownFilterHeight>(
