...
#define EXTRACT_AND_VERIFY_DIMENSIONS(label)                                   \
  ...
  VLOG(2) << "GradDepthwiseConv2d: " << label << " Input: [" << batch << ", "      \
...
//
template <typename T>
static void CopyOutputBackpropRegion(const GradDepthwiseArgs& args,
...
template <typename T>
static void ComputeBackpropInput(const GradDepthwiseArgs& args,
                                 const int64 padded_filter_inner_dim_size,
                                 const int64 in_r, const int64 in_c,
...
template <typename T>
struct LaunchGradDepthwiseConvBackpropInputOp<CPUDevice, T> {
  typedef typename Eigen::internal::packet_traits<T>::type Packet;

  void operator()(OpKernelContext* ctx, const GradDepthwiseArgs& args,
    ...
    if (pad_filter) {
      ...
      functor::GradDepthwiseFilterPadOp<T>()(
...
template <typename T>
static void GradDepthwiseConvBackpropInputReference(const GradDepthwiseArgs& args,
...
extern template struct LaunchGradDepthwiseConvBackpropInputOp<GPUDevice,
                                                          Eigen::half>;
extern template struct LaunchGradDepthwiseConvBackpropInputOp<GPUDevice, float>;
extern template struct LaunchGradDepthwiseConvBackpropInputOp<GPUDevice, double>;

#endif  // GOOGLE_CUDA

// Kernel to compute the input backprop for depthwise convolution.
template <typename Device, class T>
class GradDepthwiseConv2dNativeBackpropInputOp : public OpKernel {
 public:
  explicit GradDepthwiseConv2dNativeBackpropInputOp(OpKernelConstruction* context)
      : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {
    ...
    EXTRACT_AND_VERIFY_DIMENSIONS("GradDepthwiseConv2DBackpropInput");
    ...
    LaunchGradDepthwiseConvBackpropInputOp<Device, T>()(
...
REGISTER_KERNEL_BUILDER(Name("GradDepthwiseConv2dNativeBackpropInput")
                        ...
                        GradDepthwiseConv2dNativeBackpropInputOp<GPUDevice, float>);

REGISTER_KERNEL_BUILDER(
    Name("GradDepthwiseConv2dNativeBackpropInput")
        .Device(DEVICE_GPU)
        .TypeConstraint<double>("T")
        .HostMemory("input_sizes"),
    GradDepthwiseConv2dNativeBackpropInputOp<GPUDevice, double>);
#endif  // GOOGLE_CUDA
...
//
template <typename T>
static void ComputeBackpropFilter(const GradDepthwiseArgs& args,
...
struct LaunchGradDepthwiseConvBackpropFilterOp;

template <typename T>
struct LaunchGradDepthwiseConvBackpropFilterOp<CPUDevice, T> {
  typedef typename Eigen::internal::packet_traits<T>::type Packet;
...
template <typename T>
static void GradDepthwiseConvBackpropFilterReference(const GradDepthwiseArgs& args,
                                                 const T* out_backprop,
                                                 const T* input,
                                                 T* filter_backprop) {
...
extern template struct LaunchGradDepthwiseConvBackpropFilterOp<GPUDevice,
                                                           Eigen::half>;
extern template struct LaunchGradDepthwiseConvBackpropFilterOp<GPUDevice, float>;
extern template struct LaunchGradDepthwiseConvBackpropFilterOp<GPUDevice, double>;

#endif  // GOOGLE_CUDA

// Kernel to compute the filter backprop for depthwise convolution.
template <typename Device, class T>
class GradDepthwiseConv2dNativeBackpropFilterOp : public OpKernel {
 public:
  explicit GradDepthwiseConv2dNativeBackpropFilterOp(OpKernelConstruction* context)
      : OpKernel(context) {
    OP_REQUIRES_OK(context, context->GetAttr("strides", &strides_));
    OP_REQUIRES(context, strides_.size() == 4,
                errors::InvalidArgument("Sliding window strides field must "
  ...
  void Compute(OpKernelContext* context) override {
    ...
    EXTRACT_AND_VERIFY_DIMENSIONS("GradDepthwiseConv2DBackpropFilter");
    ...
    LaunchGradDepthwiseConvBackpropFilterOp<Device, T>()(
...
REGISTER_KERNEL_BUILDER(
    ...
    GradDepthwiseConv2dNativeBackpropFilterOp<GPUDevice, float>);
...
REGISTER_KERNEL_BUILDER(
    Name("GradDepthwiseConv2dNativeBackpropFilter")
        ...
        .TypeConstraint<double>("T")
        .HostMemory("filter_sizes"),
    GradDepthwiseConv2dNativeBackpropFilterOp<GPUDevice, double>);
#endif  // GOOGLE_CUDA
