...
struct LaunchGradDepthwiseConvOp {
...
template <typename Device, typename T>
struct LaunchGradDepthwiseConvBackpropInputOp {
  void operator()(OpKernelContext* ctx, const GradDepthwiseArgs& args,
...
#if GOOGLE_CUDA
template <typename T>
struct LaunchGradDepthwiseConvOp<Eigen::GpuDevice, T> {
  void operator()(OpKernelContext* ctx, const GradDepthwiseArgs args,
                  const T* input, const T* filter, T* output,
                  TensorFormat data_format);
};
...
template <typename T>
struct LaunchGradDepthwiseConvBackpropInputOp<Eigen::GpuDevice, T> {
  void operator()(class OpKernelContext* ctx, const GradDepthwiseArgs& args,
                  const T* out_backprop, const T* filter, T* in_backprop,
                  TensorFormat data_format);
};

template <typename T>
struct LaunchGradDepthwiseConvBackpropFilterOp<Eigen::GpuDevice, T> {
  void operator()(class OpKernelContext* ctx, const GradDepthwiseArgs& args,
                  const T* out_backprop, const T* input, T* filter_backprop,
                  TensorFormat data_format);
};
...
template <typename T>
struct GradDepthwiseFilterPadOp {
  void operator()(const GradDepthwiseArgs& args, const T* filter,
                  T* padded_filter) {
...
struct DepthwiseInputCopyOp {
  void operator()(const GradDepthwiseArgs& args,
