...
template <typename T>
struct HexRotDepthwiseConv2DKernel {
  static void Run(const HexRotDepthwiseArgs& args,
...
// HexRotDepthwiseConv2DKernel for details).
//
...
template <typename T>
struct LaunchHexRotDepthwiseConvOp<CPUDevice, T> {
  typedef typename Eigen::internal::packet_traits<T>::type Packet;
    ...
    auto shard = [&ctx, &args, &input, &filter_data, &output, data_format](
      ...
      for (int64 i = start; i < limit; ++i) {
        ...
        for (int64 out_c = 0; out_c < args.out_cols; ++out_c) {
          ...
          functor::HexRotDepthwiseInputCopyOp<T>()(args, padded_filter_inner_dim_size,
                                             out_r, out_c, input + in_base,
                                             input_buffer_data);

          // Process buffered input across all filters and store to output.
          HexRotDepthwiseConv2DKernel<T>::Run(
...
class HexRotDepthwiseConv2dNativeOp : public BinaryOp<T> {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    LaunchHexRotDepthwiseConvOp<Device, T>()(context, args, input_ptr, filter_ptr,
