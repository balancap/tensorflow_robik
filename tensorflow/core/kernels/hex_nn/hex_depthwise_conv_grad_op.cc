...
//
template <typename T>
static void CopyOutputBackpropRegion(const HexDepthwiseArgs& args,
...
template <typename T>
static void ComputeBackpropInput(const HexDepthwiseArgs& args,
                                 const int64 padded_filter_inner_dim_size,
                                 const int64 in_r, const int64 in_c,
...
template <typename T>
struct LaunchHexDepthwiseConvBackpropInputOp<CPUDevice, T> {
  typedef typename Eigen::internal::packet_traits<T>::type Packet;

  void operator()(OpKernelContext* ctx, const HexDepthwiseArgs& args,
...
template <typename T>
static void DepthwiseConvBackpropInputReference(const HexDepthwiseArgs& args,
...
class HexDepthwiseConv2dNativeBackpropInputOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    LaunchHexDepthwiseConvBackpropInputOp<Device, T>()(
...
//
template <typename T>
static void ComputeBackpropFilter(const HexDepthwiseArgs& args,
...
struct LaunchHexDepthwiseConvBackpropFilterOp;

template <typename T>
struct LaunchHexDepthwiseConvBackpropFilterOp<CPUDevice, T> {
  typedef typename Eigen::internal::packet_traits<T>::type Packet;
      ...
      for (int b = start; b < limit; ++b) {
        ...
        for (int out_r = 0; out_r < args.out_rows; ++out_r) {
          for (int out_c = 0; out_c < args.out_cols; ++out_c) {
            ...
            functor::HexDepthwiseInputCopyOp<T>()(
...
class HexDepthwiseConv2dNativeBackpropFilterOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    LaunchHexDepthwiseConvBackpropFilterOp<Device, T>()(
