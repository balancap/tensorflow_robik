...
template <typename T>
struct GradDepthwiseConv2DKernel {
  static void Run(const GradDepthwiseArgs& args,
...
// GradDepthwiseConv2DKernel for details).
//
...
template <typename T>
struct LaunchGradDepthwiseConvOp<CPUDevice, T> {
  typedef typename Eigen::internal::packet_traits<T>::type Packet;
    ...
    auto shard = [&ctx, &args, &input, &filter_data, &output, data_format](
      ...
      for (int64 i = start; i < limit; ++i) {
        ...
        for (int64 out_c = 0; out_c < args.out_cols; ++out_c) {
          ...
          GradDepthwiseConv2DKernel<T>::Run(
...
extern template struct LaunchGradDepthwiseConvOp<GPUDevice, Eigen::half>;
extern template struct LaunchGradDepthwiseConvOp<GPUDevice, float>;
extern template struct LaunchGradDepthwiseConvOp<GPUDevice, double>;
...
class GradDepthwiseConv2dNativeOp : public BinaryOp<T> {
 public:
  explicit GradDepthwiseConv2dNativeOp(OpKernelConstruction* context)
      : BinaryOp<T>(context) {
  ...
  void Compute(OpKernelContext* context) override {
    ...
    VLOG(2) << "GradDepthwiseConv2dNative: "
    ...
    LaunchGradDepthwiseConvOp<Device, T>()(context, args, input_ptr, filter_ptr,
...
#if GOOGLE_CUDA
REGISTER_KERNEL_BUILDER(
    Name("GradDepthwiseConv2dNative").Device(DEVICE_GPU).TypeConstraint<Eigen::half>("T"),
    GradDepthwiseConv2dNativeOp<GPUDevice, Eigen::half>);

REGISTER_KERNEL_BUILDER(
    Name("GradDepthwiseConv2dNative").Device(DEVICE_GPU).TypeConstraint<float>("T"),
    GradDepthwiseConv2dNativeOp<GPUDevice, float>);

REGISTER_KERNEL_BUILDER(Name("GradDepthwiseConv2dNative")
                            .Device(DEVICE_GPU)
                            .TypeConstraint<double>("T"),
                        GradDepthwiseConv2dNativeOp<GPUDevice, double>);
#endif
