...
#include "grad_depthwise_conv_op.h"
...
// The GradDepthwiseConv2dGPUKernels perform either forward or backprop input
// convolution depending on a template argument of this enum.
enum GradDepthwiseConv2dDirection { DIRECTION_FORWARD, DIRECTION_BACKWARD };


// -------------------------------------------------------------------------- //
// Implementation of depthwise conv2d.
// -------------------------------------------------------------------------- //
...
__global__ void __launch_bounds__(1024, 2)
    GradDepthwiseConv2dGPUKernelNHWC(const GradDepthwiseArgs args, const T* input,
...
__global__ void __launch_bounds__(640, 2)
    GradDepthwiseConv2dBackpropInputGPUKernelNHWC(const GradDepthwiseArgs args,
...
template <typename T, int kKnownFilterWidth, int kKnownFilterHeight,
          int kKnownDepthMultiplier>
__global__ void __launch_bounds__(640, 2)
    GradDepthwiseConv2dBackpropFilterGPUKernelNHWC(const GradDepthwiseArgs args,
                                               const T* out_backprop,
                                               const T* input,
                                               T* filter_backprop,
...
EIGEN_DEVICE_FUNC bool CanLaunchGradDepthwiseConv2dGPUSmall(
    const GradDepthwiseArgs args) {
...
EIGEN_DEVICE_FUNC bool CanLaunchGradDepthwiseConv2dBackpropFilterGPUSmall(
    const GradDepthwiseArgs args, const int block_rows) {
  return false && args.depth_multiplier == 1 && args.stride == 1 && args.in_rows <= 32 &&
         args.in_cols <= 32 && args.in_rows == args.out_rows &&
         args.in_cols == args.out_cols && args.pad_rows >= 0 &&
         args.pad_rows < args.filter_rows && args.pad_cols >= 0 &&
...
// use this kernel if CanLaunchGradDepthwiseConv2dGPUSmall(args) returns true.
...
// rotated by 180°.
template <typename T, GradDepthwiseConv2dDirection kDirection,
          int kKnownFilterWidth, int kKnownFilterHeight, int kBlockSlices,
          bool kKnownEvenRows>
__global__ __launch_bounds__(1024, 2) void GradDepthwiseConv2dGPUKernelNHWCSmall(
    const GradDepthwiseArgs args, const T* input, const T* filter, T* output) {
  assert(CanLaunchGradDepthwiseConv2dGPUSmall(args));
...
template <typename T, int kKnownFilterWidth, int kKnownFilterHeight,
          int kKnownDepthMultiplier>
__global__ void __launch_bounds__(1024, 2)
    GradDepthwiseConv2dGPUKernelNCHW(const GradDepthwiseArgs args, const T* input,
                                 const T* filter, T* output, int num_outputs) {
  const int in_rows = args.in_rows;
  const int in_cols = args.in_cols;
  const int in_depth = args.in_depth;
...
template <typename T, GradDepthwiseConv2dDirection kDirection,
          int kKnownFilterWidth, int kKnownFilterHeight, int kBlockSlices,
          bool kKnownEvenRows>
__global__ __launch_bounds__(1024, 2) void GradDepthwiseConv2dGPUKernelNCHWSmall(
    const GradDepthwiseArgs args, const T* input, const T* filter, T* output) {
  assert(CanLaunchGradDepthwiseConv2dGPUSmall(args));
...
template <typename T, GradDepthwiseConv2dDirection kDirection,
          int kKnownFilterWidth, int kKnownFilterHeight, int kBlockSlices,
          bool kKnownEvenRows>
void LaunchGradDepthwiseConv2dGPUSmall(const GpuDevice& d, const GradDepthwiseArgs args,
                                   const T* input, const T* filter, T* output,
                                   TensorFormat data_format) {
  const int block_rows = (args.in_rows + 1) / 2;
  dim3 block_dim;
  void (*kernel)(const GradDepthwiseArgs, const T*, const T*, T*);
  if (data_format == FORMAT_NHWC) {
    block_dim = dim3(kBlockSlices, args.in_cols, block_rows);
    kernel = GradDepthwiseConv2dGPUKernelNHWCSmall<T, kDirection, kKnownFilterWidth,
                                               kKnownFilterHeight, kBlockSlices,
                                               kKnownEvenRows>;
  } else if (data_format == FORMAT_NCHW) {
    block_dim = dim3(args.in_cols, block_rows, kBlockSlices);
    kernel = GradDepthwiseConv2dGPUKernelNCHWSmall<T, kDirection, kKnownFilterWidth,
                                               kKnownFilterHeight, kBlockSlices,
                                               kKnownEvenRows>;
  } else {
...
void LaunchGradDepthwiseConv2dGPUSmall(const GpuDevice& d, const GradDepthwiseArgs args,
                                   ...
                                   TensorFormat data_format) {
  if (args.in_rows & 1) {
    LaunchGradDepthwiseConv2dGPUSmall<T, kDirection, kKnownFilterWidth,
...
template <typename T, int kKnownFilterWidth, int kKnownFilterHeight,
          int kKnownDepthMultiplier>
void LaunchGradDepthwiseConv2dGPU(const GpuDevice& d, const GradDepthwiseArgs args,
                              const T* input, const T* filter, T* output,
                              TensorFormat data_format) {
  void (*kernel)(const GradDepthwiseArgs, const T*, const T*, T*, int);
  if (data_format == FORMAT_NHWC) {
    kernel =
        GradDepthwiseConv2dGPUKernelNHWC<T, kKnownFilterWidth, kKnownFilterHeight,
                                     kKnownDepthMultiplier>;
  } else if (data_format == FORMAT_NCHW) {
    kernel =
        GradDepthwiseConv2dGPUKernelNCHW<T, kKnownFilterWidth, kKnownFilterHeight,
                                     kKnownDepthMultiplier>;
  } else {
    assert(false && "Incorrect data format");
    return;
...
void LaunchGradDepthwiseConv2dGPU(const GpuDevice& d, const GradDepthwiseArgs args,
  ...
  if (args.depth_multiplier == 1) {
    if (CanLaunchGradDepthwiseConv2dGPUSmall(args)) {
      LaunchGradDepthwiseConv2dGPUSmall<T, DIRECTION_FORWARD, kKnownFilterWidth,
                                    kKnownFilterHeight>(d, args, input, filter,
                                                        output, data_format);
      return;
    }

    LaunchGradDepthwiseConv2dGPU<T, kKnownFilterWidth, kKnownFilterHeight, 1>(
        d, args, input, filter, output, data_format);
  } else {
    LaunchGradDepthwiseConv2dGPU<T, kKnownFilterWidth, kKnownFilterHeight, -1>(
        d, args, input, filter, output, data_format);
...
template <typename T>
void LaunchGradDepthwiseConvOp<GPUDevice, T>::operator()(OpKernelContext* ctx,
                                                     const GradDepthwiseArgs args,
                                                     const T* input,
  ...
  if (args.filter_rows == 3 && args.filter_cols == 3) {
    LaunchGradDepthwiseConv2dGPU<T, 3, 3>(d, args, input, filter, output,
                                      data_format);
  } else {
    LaunchGradDepthwiseConv2dGPU<T, -1, -1>(d, args, input, filter, output,
                                        data_format);
  }
  auto stream = ctx->op_device_context()->stream();
  OP_REQUIRES(ctx, stream->ok(),
              errors::Internal(
                  "Launch of gpu kernel for GradDepthwiseConv2dGPULaunch failed"));
}

template struct LaunchGradDepthwiseConvOp<GPUDevice, Eigen::half>;
template struct LaunchGradDepthwiseConvOp<GPUDevice, float>;
template struct LaunchGradDepthwiseConvOp<GPUDevice, double>;
...
template <typename T, int kKnownFilterWidth, int kKnownFilterHeight,
          int kKnownDepthMultiplier>
__global__ void __launch_bounds__(640, 2)
    GradDepthwiseConv2dBackpropInputGPUKernelNCHW(const GradDepthwiseArgs args,
                                              const T* out_backprop,
                                              const T* filter, T* in_backprop,
                                              int num_in_backprop) {
  const int in_rows = args.in_rows;
...
template <typename T, int kKnownFilterWidth, int kKnownFilterHeight,
          int kKnownDepthMultiplier>
void LaunchGradDepthwiseConv2dBackpropInputGPU(const GpuDevice& d,
                                           const GradDepthwiseArgs args,
                                           const T* out_backprop,
                                           const T* filter, T* in_backprop,
                                           TensorFormat data_format) {
  void (*kernel)(const GradDepthwiseArgs, const T*, const T*, T*, int);
  if (data_format == FORMAT_NHWC) {
    kernel = GradDepthwiseConv2dBackpropInputGPUKernelNHWC<
        T, kKnownFilterWidth, kKnownFilterHeight, kKnownDepthMultiplier>;
  } else if (data_format == FORMAT_NCHW) {
    kernel = GradDepthwiseConv2dBackpropInputGPUKernelNCHW<
        T, kKnownFilterWidth, kKnownFilterHeight, kKnownDepthMultiplier>;
  } else {
    assert(false && "Incorrect data format");
    return;
...
void LaunchGradDepthwiseConv2dBackpropInputGPU(const GpuDevice& d,
  ...
  if (args.depth_multiplier == 1) {
    if (CanLaunchGradDepthwiseConv2dGPUSmall(args)) {
      LaunchGradDepthwiseConv2dGPUSmall<T, DIRECTION_BACKWARD, kKnownFilterWidth,
                                    kKnownFilterHeight>(
          d, args, out_backprop, filter, in_backprop, data_format);
      return;
    }

    LaunchGradDepthwiseConv2dBackpropInputGPU<T, kKnownFilterWidth,
...
// A simple launch pad to launch the Cuda kernel for depthwise convolution.
template <typename T>
void LaunchGradDepthwiseConvBackpropInputOp<GPUDevice, T>::operator()(
    OpKernelContext* ctx, const GradDepthwiseArgs& args, const T* out_backprop,
    const T* filter, T* in_backprop, TensorFormat data_format) {
  const GPUDevice& d = ctx->eigen_device<GPUDevice>();
  if (args.filter_rows == 3 && args.filter_cols == 3) {
    LaunchGradDepthwiseConv2dBackpropInputGPU<T, 3, 3>(
        d, args, out_backprop, filter, in_backprop, data_format);
  } else {
    LaunchGradDepthwiseConv2dBackpropInputGPU<T, -1, -1>(
        d, args, out_backprop, filter, in_backprop, data_format);
  }
  auto stream = ctx->op_device_context()->stream();
  OP_REQUIRES(ctx, stream->ok(),
              errors::Internal("Launch of gpu kernel for "
                               "GradDepthwiseConv2dBackpropInp"
                               "utGPULaunch failed"));
}

template struct LaunchGradDepthwiseConvBackpropInputOp<GPUDevice, Eigen::half>;
template struct LaunchGradDepthwiseConvBackpropInputOp<GPUDevice, float>;
template struct LaunchGradDepthwiseConvBackpropInputOp<GPUDevice, double>;
...
// CanLaunchGradDepthwiseConv2dGPUSmall(args) returns true.
...
__global__
__launch_bounds__(1024, 2) void GradDepthwiseConv2dBackpropFilterGPUKernelNHWCSmall(
    const GradDepthwiseArgs args, const T* output, const T* input, T* filter) {
  assert(CanLaunchGradDepthwiseConv2dBackpropFilterGPUSmall(args, blockDim.z));
  // Holds block plus halo and filter data for blockDim.x depths.
  extern __shared__ __align__(sizeof(T)) unsigned char shared_memory[];
  T* const shared_data = reinterpret_cast<T*>(shared_memory);
...
template <typename T, int kKnownFilterWidth, int kKnownFilterHeight,
          int kKnownDepthMultiplier>
__global__ void __launch_bounds__(640, 2)
    GradDepthwiseConv2dBackpropFilterGPUKernelNCHW(const GradDepthwiseArgs args,
                                               const T* out_backprop,
                                               const T* input,
                                               T* filter_backprop,
                                               int num_out_backprop) {
...
template <typename T, int kKnownFilterWidth, int kKnownFilterHeight,
          int kBlockSlices, int kAccumPixels>
__global__
__launch_bounds__(1024, 2) void GradDepthwiseConv2dBackpropFilterGPUKernelNCHWSmall(
    const GradDepthwiseArgs args, const T* output, const T* input, T* filter) {
  assert(CanLaunchGradDepthwiseConv2dBackpropFilterGPUSmall(args, blockDim.x));
  // Holds block plus halo and filter data for blockDim.z depths.
  extern __shared__ __align__(sizeof(T)) unsigned char shared_memory[];
  T* const shared_data = reinterpret_cast<T*>(shared_memory);
...
template <typename T, int kKnownFilterWidth, int kKnownFilterHeight,
          int kBlockSlices, int kAccumPixels>
bool TryLaunchGradDepthwiseConv2dBackpropFilterGPUSmall(
    const GpuDevice& d, const GradDepthwiseArgs args, const int block_rows,
  ...
  if (data_format == FORMAT_NHWC) {
    block_dim = dim3(kBlockSlices, args.in_cols, block_rows);
    kernel = GradDepthwiseConv2dBackpropFilterGPUKernelNHWCSmall<
        T, kKnownFilterWidth, kKnownFilterHeight, kBlockSlices, kAccumPixels>;
  } else if (data_format == FORMAT_NCHW) {
    block_dim = dim3(args.in_cols, block_rows, kBlockSlices);
    kernel = GradDepthwiseConv2dBackpropFilterGPUKernelNCHWSmall<
        T, kKnownFilterWidth, kKnownFilterHeight, kBlockSlices, kAccumPixels>;
  } else {
    assert(false && "Incorrect data format");
    return false;
...
bool TryLaunchGradDepthwiseConv2dBackpropFilterGPUSmall(
  ...
  if (block_pixels > 512) {
    return TryLaunchGradDepthwiseConv2dBackpropFilterGPUSmall<
...
template <typename T, int kKnownFilterWidth, int kKnownFilterHeight>
bool TryLaunchGradDepthwiseConv2dBackpropFilterGPUSmall(
    const GpuDevice& d, const GradDepthwiseArgs args, const T* out_backprop,
  ...
  if (!CanLaunchGradDepthwiseConv2dBackpropFilterGPUSmall(args, block_rows)) {
    return false;
  }
...
template <typename T, int kKnownFilterWidth, int kKnownFilterHeight,
          int kKnownDepthMultiplier>
void LaunchGradDepthwiseConv2dBackpropFilterGPU(const GpuDevice& d,
                                            const GradDepthwiseArgs args,
                                            const T* out_backprop,
  ...
  void (*kernel)(const GradDepthwiseArgs, const T*, const T*, T*, int);
  if (data_format == FORMAT_NHWC) {
    kernel = GradDepthwiseConv2dBackpropFilterGPUKernelNHWC<
        T, kKnownFilterWidth, kKnownFilterHeight, kKnownDepthMultiplier>;
  } else if (data_format == FORMAT_NCHW) {
    kernel = GradDepthwiseConv2dBackpropFilterGPUKernelNCHW<
        T, kKnownFilterWidth, kKnownFilterHeight, kKnownDepthMultiplier>;
  } else {
    assert(false && "Incorrect data format");
    return;
...
void LaunchGradDepthwiseConv2dBackpropFilterGPU(const GpuDevice& d,
                                            ...
                                            TensorFormat data_format) {
  if (args.depth_multiplier == 1) {
    if (TryLaunchGradDepthwiseConv2dBackpropFilterGPUSmall<T, kKnownFilterWidth,
                                                       kKnownFilterHeight>(
            d, args, out_backprop, input, filter_backprop, data_format)) {
      return;
    }

    LaunchGradDepthwiseConv2dBackpropFilterGPU<T, kKnownFilterWidth,
                                           kKnownFilterHeight, 1>(
        d, args, out_backprop, input, filter_backprop, data_format);
  } else {
    LaunchGradDepthwiseConv2dBackpropFilterGPU<T, kKnownFilterWidth,
...
// A simple launch pad to launch the Cuda kernel for depthwise convolution.
template <typename T>
void LaunchGradDepthwiseConvBackpropFilterOp<GPUDevice, T>::operator()(
    OpKernelContext* ctx, const GradDepthwiseArgs& args, const T* out_backprop,
    const T* input, T* filter_backprop, TensorFormat data_format) {
  const GPUDevice& d = ctx->eigen_device<GPUDevice>();
  auto stream = ctx->op_device_context()->stream();
  ...
  if (args.filter_rows == 3 && args.filter_cols == 3) {
    LaunchGradDepthwiseConv2dBackpropFilterGPU<T, 3, 3>(
        d, args, out_backprop, input, filter_backprop, data_format);
  } else {
    LaunchGradDepthwiseConv2dBackpropFilterGPU<T, -1, -1>(
        d, args, out_backprop, input, filter_backprop, data_format);
  }
  OP_REQUIRES(ctx, stream->ok(),
              errors::Internal("Launch of gpu kernel for "
                               "GradDepthwiseConv2dBackpropFil"
                               "terGPULaunch failed"));
}

template struct LaunchGradDepthwiseConvBackpropFilterOp<GPUDevice, Eigen::half>;
template struct LaunchGradDepthwiseConvBackpropFilterOp<GPUDevice, float>;
template struct LaunchGradDepthwiseConvBackpropFilterOp<GPUDevice, double>;
}  // namespace tensorflow
#endif  // GOOGLE_CUDA
