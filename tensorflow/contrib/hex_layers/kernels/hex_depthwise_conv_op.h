...
struct LaunchHexDepthwiseConvOp {
...
template <typename Device, typename T>
struct LaunchHexDepthwiseConvBackpropInputOp {
  void operator()(OpKernelContext* ctx, const HexDepthwiseArgs& args,
...
#if GOOGLE_CUDA
template <typename T>
struct LaunchHexDepthwiseConvOp<Eigen::GpuDevice, T> {
  void operator()(OpKernelContext* ctx, const HexDepthwiseArgs args,
                  const T* input, const T* filter, T* output,
                  TensorFormat data_format);
};
...
template <typename T>
struct LaunchHexDepthwiseConvBackpropInputOp<Eigen::GpuDevice, T> {
  void operator()(class OpKernelContext* ctx, const HexDepthwiseArgs& args,
                  const T* out_backprop, const T* filter, T* in_backprop,
                  TensorFormat data_format);
};

template <typename T>
struct LaunchHexDepthwiseConvBackpropFilterOp<Eigen::GpuDevice, T> {
  void operator()(class OpKernelContext* ctx, const HexDepthwiseArgs& args,
                  const T* out_backprop, const T* input, T* filter_backprop,
                  TensorFormat data_format);
};
...
template <typename T>
struct HexDepthwiseFilterPadOp {
  void operator()(const HexDepthwiseArgs& args, const T* filter,
                  T* padded_filter) {
...
template <typename T>
struct HexDepthwiseInputCopyOp {
  void operator()(const HexDepthwiseArgs& args,
