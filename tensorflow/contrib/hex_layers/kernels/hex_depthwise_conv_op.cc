...
template <typename T>
struct LaunchHexDepthwiseConvOp<CPUDevice, T> {
  typedef typename Eigen::internal::packet_traits<T>::type Packet;
    ...
    auto shard = [&ctx, &args, &input, &filter_data, &output, data_format](
      ...
      for (int64 i = start; i < limit; ++i) {
        ...
        for (int64 out_c = 0; out_c < args.out_cols; ++out_c) {
          ...
          functor::HexDepthwiseInputCopyOp<T>()(args, padded_filter_inner_dim_size,
...
extern template struct LaunchHexDepthwiseConvOp<GPUDevice, Eigen::half>;
extern template struct LaunchHexDepthwiseConvOp<GPUDevice, float>;
extern template struct LaunchHexDepthwiseConvOp<GPUDevice, double>;
...
class HexDepthwiseConv2DNativeOp : public BinaryOp<T> {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    LaunchHexDepthwiseConvOp<Device, T>()(context, args, input_ptr, filter_ptr,
